...
void ParCSRMatrix::print_tap_mult(const aligned_vector<int>& proc_distances)
{
    int rank, rank_node, rank_socket;
    int ranks_per_socket;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    rank_node = comm->topology->get_node(rank);
    ranks_per_socket = comm->topology->PPN / 2;
    int local_rank = rank % comm->topology->PPN;
...
void ParCSRMatrix::print_tap_mult_T(const aligned_vector<int>& proc_distances)
{
    int rank, rank_node, rank_socket;
    int ranks_per_socket;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    rank_node = partition->topology->get_node(rank);
    ranks_per_socket = partition->topology->PPN / 2;
    int local_rank = rank % partition->topology->PPN;
    rank_socket = local_rank / ranks_per_socket;
    ...
    for (int i = 0; i < comm_G->recv_data->num_msgs; i++)
    {
        start = comm_G->recv_data->indptr[i];
        end = comm_G->recv_data->indptr[i+1];
        proc = comm_G->recv_data->procs[i];
        size = (end - start)*sizeof(double);
        byte_hops += (size * proc_distances[proc]);
        num_G++;
